...
const SideMenu = () => {
  ...
  return (
    <aside className="sideMenuHeight bg-primary sticky top-4 left-0 py-4 pl-4 px-8 rounded-lg flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-4">
              {doctorLinks.map((link, index) => (
                <Link key={index} to={`/course/${link.link}`}>
                  <li  className="hover:bg-white duration-200 p-2 rounded-lg">
                    {link.name}
                  </li>
                </Link>
              ))}
